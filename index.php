...
if (isset($_POST['file_ids']) && is_array($_POST['file_ids'])) {
    ...
    if (!$conn->connect_error) {
        foreach ($_POST['file_ids'] as $delete_id) {
            ...
            // ❗ Don't delete the file physically here!
    ...
    if (isset($_POST['restore_id'])) {
        ...
        if ($res && $row = $res->fetch_assoc()) {
            ...
            $conn->query("DELETE FROM trash WHERE id = $id");
             logAction($conn, $id, $username, "Restored");
        }
        header("Location: " . $_SERVER['PHP_SELF']);
        exit;
    }
        ...
        <div class="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
          ...
          <div class="flex items-center space-x-2">
            ...
            <div class="relative w-full md:w-64">
              ...
              <input type="text" name="search" placeholder="Search files..." value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>"
                 class="border border-gray-300 px-4 py-2 rounded-l w-full max-w-xs focus:outline-none focus:ring-2 focus:ring-teal-500">
               <button type="submit" class="bg-teal-600 text-white px-4 py-2 rounded-r hover:bg-teal-700">
                 Search
                </button>
                </div>
              </form>
       ...
       <button type="button" class="hidden md:flex items-center px-3 py-2 border border-teal-500 rounded-md text-sm text-teal-700 hover:bg-teal-50" onclick="submitDownload()"><i class="fas fa-download mr-2"></i>Download </button>
            ...
            <form method="POST" id="delete-selected-form" onsubmit="return confirm('Are you sure you want to delete selected files?');">
              ...
              <div class="relative md:hidden">
                <button id="mobile-action-toggle" class="flex items-center px-3 py-2 border border-teal-500 rounded-md text-sm text-teal-700 hover:bg-teal-50">
                  <i class="fas fa-ellipsis-h"></i>
                </button>
                <div id="mobile-action-menu" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 hidden">
                  <div class="py-1">
                    <a href="#" class="flex items-center px-4 py-2 text-sm hover:bg-teal-50 text-teal-700" id="mobile-upload-btn">
                      <i class="fas fa-upload mr-2 text-teal-600"></i>
                      <span>Upload</span>
                    </a>
                    <a href="#" class="flex items-center px-4 py-2 text-sm hover:bg-teal-50 text-teal-700 mobile-action-item" data-requires-selection="true" id="mobile-download-btn">
                      <i class="fas fa-download mr-2 text-teal-600"></i>
                      <span>Download</span>
                    </a>
                    <a href="#" class="flex items-center px-4 py-2 text-sm hover:bg-teal-50 text-teal-700 mobile-action-item" data-requires-selection="true" id="mobile-approve-btn">
                      <i class="fas fa-check-circle mr-2 text-teal-600"></i>
                      <span>Approve</span>
                    </a>
                    <a href="#" class="flex items-center px-4 py-2 text-sm hover:bg-teal-50 text-teal-700 mobile-action-item" data-requires-selection="true" id="mobile-delete-btn">
...
echo '</td>';
    ...
    echo '</tr>';
                        ...
                        echo '<tr><td colspan="5" class="px-4 py-3 text-center text-gray-500">No files uploaded.</td></tr>';
                    }
                    $conn->close();
                }
                ?>
